...
const PagoPendiente = () => {
  ...
  const wppMsg = encodeURIComponent("Hola, hice un pago en ShadowScale y quedó pendiente. ¿Me pueden ayudar?");
    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "#0a0a0a" }}>
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          <h1 className="text-white text-2xl font-bold">Pago pendiente</h1>
          <p className="text-gray-400 text-sm leading-relaxed">
            Tu pago está en proceso de validación. Esto puede tomar unos minutos dependiendo de tu banco.
            Recibirás una confirmación cuando se complete.
          </p>
        </div>

        <div className="w-full rounded-2xl p-4 text-left flex flex-col gap-2"
          style={{ background: "#161618", border: "1px solid rgba(245,158,11,0.2)" }}>
          <p className="text-amber-400 text-xs font-semibold">¿Qué sigue?</p>
          <ul className="text-gray-400 text-sm space-y-1.5">
            <li>• Recibirás un correo de confirmación al aprobarse</li>
            <li>• Tu acceso se activa automáticamente</li>
            <li>• Si pasan más de 24h, contáctanos</li>
          </ul>
        </div>
        ...
        <div className="flex flex-col w-full gap-3">
          {wppUrl && (
            <a href={wppUrl} target="_blank" rel="noopener noreferrer"
              ...
              <MessageCircle className="w-4 h-4" /> Consultar estado por WhatsApp
            </a>
          )}
          <button onClick={() => navigate("/")}
            className="flex items-center justify-center gap-2 py-3 rounded-xl text-gray-400 font-medium text-sm hover:text-white transition-colors"
